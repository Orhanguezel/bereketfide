...
export const ScrollBackground: React.FC<ScrollBackgroundProps> = ({ backgrounds }) => {
  ...
  // Mount: body + main bg'yi transparan yap → Unmount: geri al
  useEffect(() => {
    ...
    if (main) main.style.setProperty('background', 'transparent', 'important');

    return () => {
      body.style.removeProperty('background');
      if (main) main.style.removeProperty('background');
    };
    ...
    <div className="fixed inset-0 pointer-events-none overflow-hidden" style={{ zIndex: -10 }}>
      {resolvedBackgrounds.map((bg, idx) => (
        <div
          ...
          style={{
            backgroundImage: `url("${encodeURI(bg.url).replace(/\(/g, '%28').replace(/\)/g, '%29')}")`,
