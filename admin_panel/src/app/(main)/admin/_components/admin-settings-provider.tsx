...
export function AdminSettingsProvider({ children }: { children: React.ReactNode }) {
  ...
  const { data: configRow, isLoading: configLoading } = useGetSiteSettingAdminByKeyQuery('bereketfide__ui_admin_config');
  ...
  const { data: pagesRow, isLoading: pagesLoading } = useGetSiteSettingAdminByKeyQuery({ key: 'bereketfide__ui_admin_pages', locale });
  ...
  const saveAdminConfig = useCallback(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      ...
      updateSetting({ key: 'bereketfide__ui_admin_config', value: newValue, locale: '*' });
    }, 1000);
  }, [updateSetting]);
