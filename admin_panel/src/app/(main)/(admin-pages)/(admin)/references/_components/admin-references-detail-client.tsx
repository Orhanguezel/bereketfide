...
export default function AdminReferenceDetailClient({ id }: { id: string }) {
  ...
  React.useEffect(() => {
    ...
    if (isCreateMode) {
      router.replace(`/references/new?${params.toString()}`);
    } else {
      router.replace(`/references/${encodeURIComponent(String(id))}?${params.toString()}`);
    }
  ...
  function onCancel() {
    router.push(`/references?locale=${encodeURIComponent(queryLocale || 'tr')}`);
  }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (isCreateMode) {
        ...
        router.replace(
          `/references/${encodeURIComponent(nextId)}?locale=${encodeURIComponent(loc)}`,
        );
        router.refresh();
        return;
      }
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="flex items-center justify-between pt-6">
            <Button variant="outline" onClick={() => router.push('/site-settings')}>
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
            ...
            {isCreateMode ? (
              <Badge>{t('admin.references.formHeader.modeCreate')}</Badge>
            ) : (
              <Badge variant="secondary">{t('admin.references.formHeader.modeEdit')}</Badge>
            )}
          </div>
        </div>
                ...
                <div className="grid gap-6 lg:grid-cols-12">
                  ...
                  <div className="space-y-4 lg:col-span-8">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        disabled={disabled}
                        placeholder={t('admin.references.form.websiteUrlPlaceholder')}
                      />
                    </div>
                  ...
                  <div className="space-y-4 lg:col-span-4">
                    ...
                    <AdminImageUploadField
                      ...
                      onOpenLibraryClick={() => router.push('/storage')}
                    />
