...
export default function AdminReferencesClient() {
  ...
  React.useEffect(() => {
    ...
    params.set('locale', l);
    router.replace(`/references?${params.toString()}`);
  ...
  function onCreate() {
    ...
    router.push(`/references/new?locale=${encodeURIComponent(l)}`);
  }

  function onEdit(id: string) {
    const l = localeShortClientOr(effectiveLocale  , 'tr');
    router.push(`/references/${encodeURIComponent(id)}?locale=${encodeURIComponent(l)}`);
  }
