  ...
  async function onSave() {
    ...
    try {
      if (isCreate) {
        ...
        if (res?.id) router.replace(`/offer/${encodeURIComponent(res.id)}`);
      } else if (form.id) {
  ...
  if (!isCreate && !offerQ.isFetching && offerQ.error && !offerQ.data) {
    return (
      <div className="space-y-4">
        ...
        <Button variant="outline" onClick={() => router.push('/offer')}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" onClick={() => router.push('/offer')}>
            <ArrowLeft className="mr-2 size-4" />
            {t('actions.backToList')}
          </Button>
