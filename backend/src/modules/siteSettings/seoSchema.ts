...
export function normalizeSeo(input: unknown): SeoObject {
  ...
  const titleDefault = cleanStr(raw.title_default) ?? 'Bereket Fide';
  const titleTemplate = cleanStr(raw.title_template) ?? '%s | Bereket Fide';
  const description = cleanStr(raw.description) ?? '';
  const siteName = cleanStr(raw.site_name) ?? 'Bereket Fide';
